...
void update(ll seg[][4],int low,int high,int idx,ll value,int pos,ll arr[])
{
    int mid;
    if(low==high)
    {
    ...
    else
    {
     ...
     if(idx>=low && idx<=mid)
     {
         update(seg,low,mid,idx,value,(2*pos)+1,arr);
     }
     else
     {
        update(seg,mid+1,high,idx,value,(2*pos)+2,arr);
     }
    ll left = (2*pos)+1;
        ll right  = (2*pos)+2;
        seg[pos][0] = seg[left][0] + seg[right][0];
